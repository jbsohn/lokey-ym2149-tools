...
fn load_song(input: &Path, clock_override: Option<u32>) -> Result<YmSequence, Box<dyn std::error::Error>> {
    ...
    match cli.command {
        MainCommands::Song { command } => match command {
            ...
            SongCommands::Render {
                ...
                println!("{} {}...", style("LOADING:").bold().cyan(), style(input.display()).cyan());
                ...
                if let Some(limit) = max_bytes {
                    if compiled.bytes.len() > limit {
                        ...
                        loop {
                            ...
                            sequence.frames.truncate((current_patterns - 1) * pattern_size);
                            compiled = compiler.compile_song(&sequence, compression_level, &compiler_options)?;
                            if compiled.bytes.len() <= limit {
                                break;
                            }
                        }
        ...
        MainCommands::Sfx { command } => match command {
            SfxCommands::Render { common: SfxCommonArgs { input, hz, clock, index }, output } => {
                let output_path = output.unwrap_or_else(|| {
                    let mut path = input.clone();
                ...
                println!("{} {}...", style("LOADING SFX:").bold().cyan(), style(input.display()).cyan());
            ...
            SfxCommands::Play { common: SfxCommonArgs { input, hz, clock, index } } => {
                let mut sequence = load_sfx(&input, index)?;
                if let Some(c) = clock {
                    sequence.source_clock = c;
                }
                ...
                println!("{} {} ({} Hz)...", style("LOADING SFX:").bold().cyan(), style(input.display()).cyan(), sequence.source_hz);
        ...
        MainCommands::Mix {
            ...
            println!("{} Loading song {}...", style("LOADING SONG:").bold().cyan(), style(song.display()).cyan());
            let mut song_seq = load_song(&song, clock)?;

            println!("{} Loading sound effect bank...", style("LOADING SFX:").bold().cyan());
            ...
            let samples_per_frame = (sample_rate_u32 as f64 / song_hz as f64).round().max(1.0) as usize;
            ...
            let sfx_frames_list: Vec<Arc<[SfxFrame]>> = sfx_list.iter().map(|s| s.frames.as_slice().into()).collect();
                        ...
                        while i < data.len() {
                            ...
                            if s.sample_in_frame >= samples_per_frame {
                                ...
                                if let Some(sf) = song_frames_cb.get(song_idx) {
                                    sf.apply_to_chip(&mut s_ref.chip, &mut s_ref.mixer, &mut s_ref.last_env_shape);
                                }
                                ...
                                for ch in 0..3 {
                                    if let Some(ref mut active) = s_ref.active_sfx[ch] {
                                        if active.current_idx < active.frames.len() {
                                            ...
                                            frame.apply_to_chip(&mut s_ref.chip, &mut s_ref.mixer, idx_to_ym_channel(ch));
                ...
                while let Ok(key) = key_rx.try_recv() {
                    ...
                    if matches!(key, Key::ArrowRight | Key::ArrowLeft) {
                        ...
                        let target_idx = match key {
                            Key::ArrowRight => current_idx.saturating_add(step_frames).min(total_song_frames.saturating_sub(1)),
                            Key::ArrowLeft => current_idx.saturating_sub(step_frames),
                            _ => current_idx,
                        };

                        let mut scratch_chip = Ym2149::with_clocks(song_seq.timing.master_clock_hz, sample_rate_u32);
                        let mut mixer = 0x3F;
                        let mut last_env_shape = None;
