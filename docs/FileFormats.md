...
Some legacy chiptune files using the `.ym` extension contain structure variants that are fundamentally incompatible with
single-chip register streaming:

* **`MIX1` (Stereo 2-Chip)**: A stereo format designed by Leonard/STSound that streams to two parallel YM2149 chips
  simultaneously (6 channels total). Because the physical Atari 7800 expansion board only features a single YM2149 chip,
  stereo 6-channel playbacks are unsupported.
* **`YMT1` / `YMT2` (Tracker Pattern Bytecode)**: A compressed tracker format that relies on custom 68000/Z80 assembly
  runtime macros and software synthesis rather than raw register streams. These are unsupported because the 6502
  replayer targets pure register-streaming payloads.
